...
echo [Face Color Match] Python 3.14 setup
where py >nul 2>nul || (
  echo Python Launcher was not found.
  echo Install CPython 3.14 from python.org with "Python Launcher" enabled.
  pause
  exit /b 1
)
...
py -3.14 -c "import cv2,numpy; assert hasattr(cv2,'FaceDetectorYN_create'); print('Existing modules OK:', 'OpenCV',cv2.__version__,'NumPy',numpy.__version__)" >nul 2>nul
if errorlevel 1 (
  echo Installing required packages into system Python 3.14...
  py -3.14 -m pip install --upgrade pip
  if errorlevel 1 goto :fail
  py -3.14 -m pip install --upgrade "numpy==2.5.2" "opencv-python-headless==4.14.0.94"
  if errorlevel 1 goto :fail
) else (
...
py -3.14 -c "import hashlib,pathlib,sys; p=pathlib.Path(r'%~dp0lib\models\face_detection_yunet_2023mar.onnx'); ok=p.is_file() and hashlib.sha256(p.read_bytes()).hexdigest()=='8f2383e4dd3cfbb4553ea8718107fc0423210dc964f9f4280604804ed2552fa4'; sys.exit(0 if ok else 1)" >nul 2>nul
if errorlevel 1 (
...
py -3.14 -c "import cv2,numpy; print('OpenCV',cv2.__version__); print('NumPy',numpy.__version__); print('FaceDetectorYN',hasattr(cv2,'FaceDetectorYN_create'))" || goto :fail

echo.
echo Installation completed for Python 3.14.
pause
exit /b 0
